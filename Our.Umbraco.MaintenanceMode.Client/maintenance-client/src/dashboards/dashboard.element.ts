...
export class MaintenanceManagerDashboard extends UmbElementMixin(LitElement) {
  ...
  constructor() {
    ...
    this.consumeContext(MAINTENANCE_CONTEXT_TOKEN, (_maintenanceContext) => {
      if (!_maintenanceContext) return;
      this.#maintenanceContexts = _maintenanceContext;

      _maintenanceContext.getStatus();
      this.observe(_maintenanceContext.status, (_status) => {
        this.status = _status;
      });
    });
  }
  ...
  render() {
    return html` <uui-box
      >${this._loaded ? this.#showDashboard() : this.#showLoader()}</uui-box
    >`;
  }
    ...
    return html`
      <div class="buttons">
        <uui-button
          ...
          look="primary"
          color="positive"
          @click=${this.#onMaintenanceToggle}
        ></uui-button>
        <uui-button
          label="Toggle Frozen"
          id="clickFrozen"
  ...
  #showAllowBackofficeToggle() {
    return html`
      <div class="switch">
        <umb-input-toggle
          .checked=${this.status?.settings?.allowBackOfficeUsersThrough ??
          false}
          .showLabels=${true}
